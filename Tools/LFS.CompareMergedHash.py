...
def compare_files(file1, file2, merged_file, output_file):
    list1 = read_file(file1)
    list2 = read_file(file2)
    merged_list = read_file(merged_file)

    combined_hashes = sorted(extract_hashes(list1) + extract_hashes(list2))
...
# ファイルパスを指定
file1_path = 'lfs-files.ModuleA.txt'
file2_path = 'lfs-files.ModuleB.txt'
merged_file_path = 'lfs-files.monorepo.txt'
output_file_path = 'discrepancies.txt'

compare_files(file1_path, file2_path, merged_file_path, output_file_path)